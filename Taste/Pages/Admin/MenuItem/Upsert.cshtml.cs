...
namespace Taste.Pages.Admin.MenuItem
{
    ...
    public class UpsertModel : PageModel
    {
        ...
        public IActionResult OnGet(int? id)
        {
            ...
            if (id != null)
            {
                MenuItemObj.MenuItem = _unitOfWork.MenuItem.GetFirstOrDefault(p => p.Id == id);
        ...
        public IActionResult OnPost()
        {
            ...
            if (MenuItemObj.MenuItem.Id == 0)
            {
                ...
                var uploades = Path.Combine(webRootPath, @"images\menuItems");
                var extention = Path.GetExtension(files[0].FileName);

                using (var fileStream = new FileStream(Path.Combine(uploades, fileName + extention), FileMode.Create))
                {
